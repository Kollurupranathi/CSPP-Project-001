...
# if the value of ARG_CNT varialble is zero, the condition will be true. 
# This makes to execute the next line 
#As the condition is true, sys.stdin.read() is stored in a variable named "TEXT"
if ARG_CNT == 1:
    filename = sys.argv[1]
    TEXT = readfile(filename)
#if the value of ARG_CNT variable is one, the condition will be true. 
#This makes to execute the next line
...
response = wc(TEXT)
#we're creating a "response" variable and saving the function wc, which will return the value which has an argument "TEXT"
print(" " + str(response[0]) + "  " + str(response[1]) + " " + str(response[2]))
#all the strings are added with spacing between them and printed
